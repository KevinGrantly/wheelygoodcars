...
public function create(): View
{
    return view('cars.create', [
        ...
        'carData' => session('car_temp_data') // <- uit session ophalen
    ]);
}
    ...
    public function verifyLicensePlate(Request $request)
    {
        ...
        try {
            ...
            if ($response->successful() && count($response->json()) > 0) {
                ...
                $request->session()->put('car_temp_data', $carData);
return view('cars.verify', ['car' => $carData]); // ✅ toon stap 2
...
{
    if ($car->user_id !== Auth::id()) {
        return redirect()->route('cars.index')->with('error', 'Geen toegang.');
    }
